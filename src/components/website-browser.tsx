            ...
            <aside className="flex flex-col gap-2 h-fit lg:sticky lg:top-4 relative lg:w-fit w-full lg:col-span-1 col-span-4">
                        ...
                        <ToggleGroup className="ml-2 gap-3" type="multiple" variant={"outline"} disabled={websitesLoading} onValueChange={(value) => searchContentSet((p) => ({
                        ...
                        }))} value={searchContent.tags}>
                            {
                                tagsList.map((tag, idx) => (
                                    <ToggleGroupItem value={tag.name} key={idx} tabIndex={0} className={cn("flex items-center p-4 !flex-0")} disabled={tag.count <= 0}>
                ...
                <Container className="bg-gradient-to-tr to-secondary-500/80 from-background-300 border-[1px] border-primary-400 text-white grid place-items-center-safe gap-3">
                    <div className="flex flex-col items-center justify-center">
                        <HeartPlus size={36} />
                        <h1 className="font-semibold text-xl text-center">Help <u>openshorts</u> Go Global!</h1>
                    </div>
