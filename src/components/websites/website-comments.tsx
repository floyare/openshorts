...
const WebsiteComments = (props: CommentsProps) => {
    ...
    const PaginationControls = memo(() => {
        ...
        return (
            <Pagination className={cn("transition-all bg-white text-text-50 dark:bg-neutral-800 dark:text-text-950 px-4 py-1 sm:w-fit w-full corner-squircle rounded-md")}>
                <PaginationContent>
                    <PaginationItem>
                        <PaginationPrevious
        ...
        <Container className="!bg-white border-1 dark:!bg-neutral-900 dark:!border-neutral-700 overflow-hidden px-6 relative space-y-4 grow h-full sm:max-w-md max-w-full">
            ...
            <div className="flex flex-col gap-2">
                ...
                {isLoading ? (<div className="space-y-2 max-h-84 overflow-y-auto">
                    <div className="w-full h-18 bg-background-800 dark:bg-neutral-700 animate-pulse corner-squircle rounded-md" />
                    <div className="w-full h-18 bg-background-800 dark:bg-neutral-700 animate-pulse corner-squircle rounded-md" />
                    <div className="w-full h-18 bg-background-800 dark:bg-neutral-700 animate-pulse corner-squircle rounded-md" />
                    <div className="w-full h-18 bg-background-800 dark:bg-neutral-700 animate-pulse corner-squircle rounded-md" />
                </div>) : (
                                ...
                                <div className="space-y-2 max-h-84 overflow-y-auto bg-neutral-200/70 dark:bg-neutral-800/50 -mx-6 px-6 -mb-4 py-6" ref={animationParent}>
                                    {
                                        slicedComments?.map((comment, index) => {
                                            ...
                                            return (
                                                <div key={index} className="flex items-start gap-2 bg-white dark:!bg-neutral-800 dark:!border-neutral-700 py-3 px-4 corner-squircle rounded-md">
                                                    <img src={comment.user.image ?? "/favicon.png"} alt={comment.created_by + "'s avatar image"} className="w-12 h-12 corner-squircle rounded-full border-[2px] border-primary-400" />
                                                    <div>
